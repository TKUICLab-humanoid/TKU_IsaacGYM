...
class WalkingGaitByLIPM:
    ...
    def process(self,stop=False):
        ...
        # print("now_step_",self.now_step_)
        ...
        if self.pre_step_ != self.now_step_:
            ...
            # print("self.now_width_",self.now_width_)
            
            self.base_x[0] = (self.footstep_x + self.zmp_x[0]) / 2
            self.base_y[0] = (self.footstep_y + self.zmp_y[0]) / 2
            # print("self.walking_state",self.walking_state)
            # print("zmp_x",self.zmp_x[0],"zmp_y",self.zmp_y[0])
            # print("base_x",self.base_x[0],"base_y",self.base_y[0])
        self.pre_step_= self.now_step_

        if stop:
            self.step = self.now_step_+1

        vx0_, vy0_,px_, py_, lpx_, lpy_, rpx_, rpy_, lpz_, rpz_, lpt_, rpt_ = self.walkinggait(self.walking_state)
        aa= [vx0_, vy0_,px_, py_]
        # print("aa",aa)
        py_u = py_
        px_u = px_
    ...
    def walkinggait(self,walking_state):
        ...
        px_, py_, lpx_, lpy_, rpx_, rpy_, lpz_, rpz_, lpt_, rpt_ = (0,) * 10
        odd_step = (self.now_step_ % 2) == 1
        
        # Common CoM velocity and position calculations
        vx0_ = self.wComVelocityInit(self.base_x[1] if walking_state in [0, 3] else 0,
                                    self.base_x[0] if walking_state in [0, 2, 3] else 0,
                                    self.zmp_x[0], self.TT_, self.Tc_)
        px_ = self.wComPosition(self.base_x[1] if walking_state in [0, 3] else 0, 
                                vx0_, self.zmp_x[0], self.t_, self.Tc_)
        vy0_ = self.wComVelocityInit(self.base_y[1] if walking_state in [0, 3] else 0,
                                    self.base_y[0] if walking_state in [0, 2, 3] else 0,
                                    self.zmp_y[0], self.TT_, self.Tc_)
        py_ = self.wComPosition(self.base_y[1] if walking_state in [0, 3] else 0,
                                vy0_, self.zmp_y[0], self.t_, self.Tc_)
        # print("Vx0",vx0_,"Vy0",vy0_,"px_",px_,"py_",py_)
        # Determine foot positions based on walking state and step parity
        if walking_state == 1:
            lpx_, rpx_ = (self.zmp_x[0], self.wFootPositionRepeat(self.now_x_[1], 0, self.t_, self.TT_, self.T_DSP_)) if odd_step else \
                        (self.wFootPositionRepeat(self.now_x_[0], 0, self.t_, self.TT_, self.T_DSP_), self.zmp_x[0])
            lpy_, rpy_ = (self.zmp_y[0], -self.wFootPositionRepeat(self.now_y_[1], 0, self.t_, self.TT_, self.T_DSP_)) if odd_step else \
                        (self.wFootPositionRepeat(self.now_y_[0], 0, self.t_, self.TT_, self.T_DSP_), self.zmp_y[0])
        elif walking_state in [0, 2, 3]:
            displacement_x = (self.displacement_x[1] + self.displacement_x[0]) / 2 if walking_state == 3 else \
                            self.displacement_x[1] + self.displacement_x[0]
            displacement_y = (self.displacement_y[1] + self.displacement_y[0]) / 2 if walking_state == 3 else \
                            self.displacement_y[1] + self.displacement_y[0]
            foot_pos_func = self.wFootPositionRepeat if walking_state == 3 else self.wFootPosition
            lpx_, rpx_ = (self.zmp_x[0], foot_pos_func(self.now_x_[1], displacement_x, self.t_, self.TT_, self.T_DSP_)) if odd_step else \
                        (foot_pos_func(self.now_x_[0], displacement_x, self.t_, self.TT_, self.T_DSP_), self.zmp_x[0])
            lpy_, rpy_ = (self.zmp_y[0], foot_pos_func(self.now_y_[1], displacement_y, self.t_, self.TT_, self.T_DSP_)) if odd_step else \
                        (foot_pos_func(self.now_y_[0], displacement_y, self.t_, self.TT_, self.T_DSP_), self.zmp_y[0])
        
        # Foot height adjustments
        lpz_ = self.wFootPositionZ(self.lift_height_, self.t_, self.TT_, self.T_DSP_) if not odd_step else 0
