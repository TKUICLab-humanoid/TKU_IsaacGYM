...
class WalkingGaitByLIPM:
    ...
    def start_step(self, robot_state, start_ids):
        print("start")
        offset = torch.zeros_like(self.py, device=self.device)
        offset[start_ids] = self.com_y_swing * torch.sin(torch.pi * self.t_[start_ids] / self.TT_[start_ids])
        ...
        self.py[start_ids] = self.wComPosition(self.zero[start_ids], self.vy0[start_ids], self.zmp_y[start_ids], self.t_[start_ids], self.Tc_[start_ids])+ offset[start_ids]
